...
public class ClientsModel extends AbstractTableModel {
	...
	private String[] columns = {"Id", "DateCreation", "NumeroTel", "Type", "data" ,"Services"};
	...
	public void update (int id,String date,String numero,TypeClient type,boolean activedata,List<ServiceActive> listeService) {
		...
		obj[i] = listeService.get(i).name();
		}
		obj[4] = activedata;
		fireTableDataChanged();
	}
	public void add(int id,String date,String numero,TypeClient type, boolean activedata, List<ServiceActive> listeService) {
		Clients clients = new Clients(id,date,numero,type, activedata,listeService);
		client.add(clients);
		rows.add( new Object [] {
				clients.getId(),
